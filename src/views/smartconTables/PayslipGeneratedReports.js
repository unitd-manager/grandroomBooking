...
import { Row, Button, Col, Input, Table, Card, CardBody, FormGroup, Label, CardHeader } from 'reactstrap';
...
function PayslipGeneratedReports() {
  ...
  const exportValue="PaySlipGeneratedReport"
  //get lineitems
  const getPaySlip = () => {
    period.month = parseInt(period.month, 10);
    period.year = parseInt(period.year, 10);
    api
      .post('/reports/getPayslipEmployeeReport', period)
      .then((res) => {
  ...
  return (
    <>
      ...
      <CardHeader className="card p-2 text-center">
        <b>Employee Payslip Generated Report</b>
      </CardHeader>
      <CardBody>
          <Row>
            <Col md="6">
